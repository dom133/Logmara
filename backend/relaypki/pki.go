...
func EnsureCA(dir string) error {
	...
	if _, err := os.Stat(caCertPath); err == nil {
		...
		// generateCA() above always yields an RSA key today - see
		// parsePrivateKeyPEM's doc comment on why loadCA (used on the other
		// branch) must additionally tolerate an EC key from an older version.
...
// parsePrivateKeyPEM loads this package's own RSA (PKCS1) keys - what every
// ca.key generated today is, whether created by EnsureCA in-process or via
// rsyslog/entrypoint.sh's relaybootstrap CLI (see backend/cmd/relaybootstrap),
// since both call this exact same package. The EC (SEC1, prime256v1) case
// only exists to keep loading a CA generated by an older version, before
// relaybootstrap replaced that container's separate openssl-based
// placeholder - see README "Rotating the relay CA" for moving such a
// deployment onto an RSA 4096 CA. Falls back to PKCS8 in case the format
// ever changes again.
