...
// LoadTailerPosition reads the last saved position and fingerprint from Redis.
// Returns (0, "", false) when client is nil or the key is missing/stale.
// The caller is responsible for validating the fingerprint against the log file.
func (c *Client) LoadTailerPosition() (pos int64, fp string, ok bool) {
	if c == nil {
		return 0, "", false
	}
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	...
	return rec.Position, rec.Fingerprint, true
}
